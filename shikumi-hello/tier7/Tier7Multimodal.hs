...
-- | Side dish: multimodal input.
--
-- A signature /input/ can carry a typed 'Image' field. When the request is
-- rendered, the image lowers to baikai's native inline image block
-- (@UserImage@), the remaining text fields render as a leading text block, and
-- the image field's name is dropped from that text — so the picture is
-- delivered as a picture, not as base64 buried in the prose.
--
-- This example renders an image-bearing request offline, inspects that the
-- user turn carries a real image block, then runs the full render -> stub ->
-- parse loop to a typed answer. ('Image' is input-only: it has no @ToSchema@,
-- so it cannot appear in an output record — that would be a compile error.)
...
-- A tiny stand-in image (the PNG magic bytes — enough to demonstrate the
-- lowering; a real run would use imageFromFile or imageFromBase64).
...
-- ---------------------------------------------------------------------------
-- Inspecting the rendered request: the image / text blocks of the user turn.
-- ---------------------------------------------------------------------------
...
-- parse driver rather than runProgram.
...
main :: IO ()
main = do
  putStrLn "tier7-multimodal: an image input the model actually sees\n"

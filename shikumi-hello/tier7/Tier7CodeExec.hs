...
-- | Side dish: code execution — @programOfThought@ and @codeAct@.
--
-- The model writes code, a sandbox runs it, and the result feeds back into a
-- typed answer. Both are ordinary @Program@s (built on @embed@), runnable
-- offline against the hermetic 'restrictedInterpreter' (a tiny
-- arithmetic/string/list DSL — no network, no filesystem, no syscalls).
--
--   * @programOfThought@ asks for a snippet that /computes/ the answer, runs
--     it, and — if it errors — feeds the error back so the model can fix it,
--     before extracting the typed answer.
--   * @codeAct@ is a ReAct-style loop whose action is a code snippet that may
--     call provided tools (via a @call("name", args)@ convention), recording a
--     trajectory.
--
-- The sandbox is a plain value captured in the embed closure — a real
-- subprocess interpreter would need IOE and so could only live outside the
-- composable-program path; the hermetic DSL fits inside any effect row.
...
main :: IO ()
main = do
  putStrLn "tier7-codeexec: the model writes code, a sandbox runs it, the result flows back\n"

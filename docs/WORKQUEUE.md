...
### Design questions & positions (resolve at review)
...
2. **Where "how to judge success" lives** (the oracle boundary). Today the verdict logic is hardcoded Haskell in `Campaign.Real`: it reads the tool-written log, looks for success banners / exit codes, and consults `knownFailuresFor arch` baselines (e.g. `["mmap3"]` for alpha) — an LLM never decides (invariant #4). When a target moves into a manifest, the question is: *does the manifest only declare facts, or may it also carry logic?*
   - **Facts-only (recommended):** the target artifact carries declarative judgment data — "command must exit 0", "log must contain marker `Build completed successfully.`", "expected known failures for this arch: [fork07, …]" — and the orchestrator keeps owning the code that *interprets* those facts. The manifest can describe what a pass looks like; it cannot inject custom judging code that could declare a lie a pass. This keeps Vector B's non-opinionated-oracle invariant intact while moving data out of Haskell.
   - **Logic-bearing:** the artifact could embed executable verdict steps (scripts / Dhall programs). More flexible, but the verdict becomes *content*, and a malformed or malicious manifest could assert success — the exact thing invariant #4 exists to prevent.
   - Position to confirm at review: facts-only, with any richer oracle types (JSON-schema, proof-hygiene) added as *new declarative fact kinds* in the schema, never as executable payloads.
3. **How hard to pin the upstream** (the dependency-commitment question). Adopting seihou means the campaign manifest layer transitively depends on Nadeem's baikai/seihou family — software we do not maintain. The question is *what exactly we lock, and what happens if upstream moves or breaks*.
   - Precedent already exists: `scripts/reconstitute.sh` pins baikai to an exact git hash (`4a9547b` = release 0.7.0.0), not a floating version.
   - **Position to confirm at review:** pin seihou (and the baikai family it needs) to exact revisions in `reconstitute.sh` the same way; no skew today (both on baikai 0.7.0.0). BSD-3-Clause permits vendoring as a fallback if the upstream ever moves, breaks, or disappears — the manifest layer is small enough that a pinned local copy is a survivable contingency, not a first choice.

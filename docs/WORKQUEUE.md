...
Last updated: 2026-09-21 (Track 10 milestones 1–5 landed; attestation mapping's 3 open questions resolved — Q1 from heartwood source — and Phase 6 steps 1–2 (attestation record + attested merge commit with receipts-as-gate-input) landed & verified end-to-end with independently recomputed hashes and 3 negative controls; steps 3–5 remain: DID-key signing, Keiro publication, verifier CLI).
...
- **arena** — **Phase 6 steps 1–2 LANDED & verified (2026-09-21)** — `Campaign.Attestation` (canonical record, `sha256:` hash, heartwood-shaped `Verification:` trailer formatter) + `approveBranch` wiring: receipts now an input (`CAMPAIGN_REVIEW_RECEIPTS` → `admitReceiptsForFiles` gate: every receipt re-derives under the oracle AND covers exactly the branch's changed files), and a clean gate attests the `--no-ff` merge commit; the verdict journal carries the attestation line. Verified end-to-end on a disposable fixture (real `unusedImportDiags` oracle output, byte-exact generated receipt): merge commit trailer present, hash independently recomputes via `sha256sum` (3/3 matches), `kioku.sessions` row matches the self-describing `keiro://` ref; negative controls all refuse with no merge + branch untouched (orphan receipt, tampered `[W-forged]` diagnostic, back-compat no-receipts). Two draft corrections landed in code: the supersede key is the merge commit's **committer date** (the hash can't be in what's signed), and the attested-time field is dropped (not re-derivable). Remaining: step 3 DID-key signing (`signer=` slot wired as `Maybe`), step 4 full-record publication to Keiro, step 5 verifier CLI.
- **arena** — **Phase 6 attestation mapping: 3 open questions RESOLVED (2026-09-21)** — Q1 from heartwood source inspection (`/home/nyc/src/heartwood` `ac607c8c`): seed normalizes commits to `CommitData` whose `Display` re-emits *all* trailers in the last message paragraph (token = alphanumerics+`-`, split on first `": "`, folded continuation lines; canonical push rules do not filter trailers) → attestation rides as a `Verification:` commit trailer (hash + keiro:// journal ref + signer DID), full record replayed from the journal (invariant #4: verifier re-derives, never trusts embedded claims). Q2: self-describing journal refs (stream name + act range embedded). Q3: supersede-by-timestamp within identity, additive, no revocation records. Phase 6 is now unblocked: trailer emission in `approveBranch`, DID-key signing, Keiro storage, verifier CLI.
- **arena** — **Track 10 CLOSED (signed off 2026-09-21)** — all six milestones confirmed: 1–2 target-manifest spike + waiver-baseline wiring (live proof), 3 repair blueprint+receipt admission (live proof + 3 tamper controls), 4 AgentPrompt artifacts (typecheck), 5 attestation mapping draft, 6 first ecosystem sweep. Deferred, not dropped: Phase 6 attestation code (steps 1–2 since landed & verified — see above; steps 3–5 = DID-key signing, Keiro publication, verifier CLI).
